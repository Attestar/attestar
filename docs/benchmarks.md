...
| ---------------------- | ---------------- | -------------- | ------------------------------ |
| Full `verify_inference` (BN254 Pairing + L Assembly) | 29,289,569 | 277,964 | 50,000,000 |
...
- **Fixture Validity**: The test fixture uses non-degenerate G2 points (from the soroban-env-host test suite) for beta/gamma/delta, non-zero G1 points for all IC entries, and a proof that genuinely satisfies the pairing equation (proof_c is computed as $-L$ so the product of pairings evaluates to 1). The recorded numbers are representative of the actual cost of verifying a real proof.
