...
Attestar. Newest entries are appended at the bottom.
...
through `attestar-common` without pulling Soroban dependencies into the prover or
RISC Zero dependencies into the contract.

## Week 2 (2026-05)

Fleshed out the `attestar-common` numeric core: fixed-point operations now cover
addition, subtraction, multiplication, and saturating variants, each with
tests. Added a shared `AttestarError` type, a quantized ReLU activation, and a
