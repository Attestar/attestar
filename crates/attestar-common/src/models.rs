...
impl TinyMLP {
    ...
    pub fn validate(&self) -> Result<(), crate::error::AttestarError> {
        if self.layers.is_empty() {
            return Err(crate::error::AttestarError::InvalidModel(
        ...
        for (_i, layer) in self.layers.iter().enumerate() {
            ...
            if layer.weights.len() != expected_weights {
                #[cfg(feature = "std")]
                return Err(crate::error::AttestarError::InvalidModel(format!(
