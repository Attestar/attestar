...
use attestar_common::models::{DenseLayer, Model, TinyMLP};
use attestar_prover::inference::{run_inference, try_run_inference};
...
fn shape_mismatch_input_length_returns_error() {
    ...
    assert_eq!(
        err,
        AttestarError::FeatureCountMismatch {
            expected: 2,
            got: 1
        }
    );
...
fn shape_mismatch_layer_chain_returns_error() {
    ...
    assert!(matches!(
        mlp.validate(),
        Err(AttestarError::InvalidModel(_))
    ));
    ...
    assert!(matches!(err, AttestarError::InvalidModel(_)));
}
