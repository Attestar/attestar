...
//!
//! Runs in the `zkvm` CI job (`RISC0_DEV_MODE=1 cargo test -p attestar-prover --features zkvm`).
...
use attestar_prover::prover::generate_receipt;
...
fn ensure_risc0_dev_mode() {
    ...
    SET.call_once(|| {
        ...
        // `cargo test -p attestar-prover --features zkvm` still uses fake receipts.
