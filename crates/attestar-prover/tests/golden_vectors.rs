...
use attestar_common::models::{DecisionTree, LogisticRegression, Model, TreeNode};
    ...
    fn try_from(model: ModelJson) -> Result<Self, String> {
        match model {
            ...
            ModelJson::TinyMLP(mlp) => {
                use attestar_common::models::DenseLayer;
                let layers: Result<Vec<DenseLayer>, String> = mlp
                    ...
                    .collect();
                Ok(Model::TinyMLP(attestar_common::models::TinyMLP {
                    layers: layers?,
                }))
            }
        }
