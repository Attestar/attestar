...
//!
//! The process-level checks use `env!("CARGO_BIN_EXE_attestar-prover")` rather than
//! a new `assert_cmd` dev-dependency.

use std::path::{Path, PathBuf};
use std::process::Command as ProcCommand;

use attestar_prover::cli::{cmd_prove, cmd_validate, CliError};
use attestar_prover::model_io::import_json;
use attestar_prover::prover::{bundle_from_json, model_commitment};
use attestar_prover::quantization::QuantizationConfig;
...
fn run_bin(args: &[&str]) -> std::process::Output {
    ProcCommand::new(env!("CARGO_BIN_EXE_attestar-prover"))
        .args(args)
        .output()
...
fn validate_passes_with_a_matching_dataset() {
    ...
    .map(|inputs| {
        let fp: Vec<_> = inputs
            .iter()
            .map(|&x| attestar_common::fixed_point::FixedPoint::quantize(x))
            .collect();
        let expected = attestar_common::inference::run_inference(&model, &fp).dequantize();
