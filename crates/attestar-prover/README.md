...
```bash
RISC0_DEV_MODE=1 cargo test -p attestar-prover --features zkvm
```
...
```bash
RISC0_SKIP_BUILD=1 cargo test -p attestar-prover
```
...
Inference lives in `attestar-common::inference` and is re-exported as
`attestar_prover::inference`. The guest depends only on `attestar-common` so host and
