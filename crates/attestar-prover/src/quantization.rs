...
) -> Result<QuantizationReport, AttestarError> {
    // Pass 1: Range check
...
fn check_parameter_range(model: &Model) -> Result<(), AttestarError> {
    match model {
...
fn check_tree_range(tree: &DecisionTree) -> Result<(), AttestarError> {
...
fn check_logistic_range(lr: &LogisticRegression) -> Result<(), AttestarError> {
...
fn check_mlp_range(mlp: &TinyMLP) -> Result<(), AttestarError> {
...
fn check_fixed_point_range(value: &i64, name: &str) -> Result<(), AttestarError> {
    if *value == i64::MIN {
        return Err(AttestarError::QuantizationError(format!(
...
fn check_overflow_bounds(model: &Model, cfg: &QuantizationConfig) -> Result<(), AttestarError> {
    match model {
...
fn check_mlp_overflow_bounds(mlp: &TinyMLP, max_input: f64) -> Result<(), AttestarError> {
...
fn validate_accuracy(
    ...
    for (inputs, expected_float_output) in validation_dataset {
        ...
        let quantized_output = attestar_common::inference::run_inference(model, &inputs_fp);
