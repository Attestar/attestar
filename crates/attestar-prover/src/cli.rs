...
use attestar_common::commitment::to_hex;
use attestar_common::error::AttestarError;
use attestar_common::fixed_point::{FixedPoint, DEFAULT_SCALE};
use attestar_common::inference::try_run_inference;
use attestar_common::models::{Model, TreeNode};
...
/// `crates/attestar-common/tests/fixed_point_props.rs`.
...
#[derive(Debug, Parser)]
#[command(
    name = "attestar-prover",
    version,
    about = "Off-chain ML inference, commitments, and proof bundles for Attestar"
)]
pub struct Cli {
...
///
/// Hand-written `Display` to match [`AttestarError`] and
...
pub enum CliError {
    ...
    InvalidModel { path: String, source: AttestarError },
    /// Inference failed (overflow, topology, cycle).
    Inference(AttestarError),
    /// Quantization validation failed.
    Validation(AttestarError),
...
mod tests_dataset {
    ...
    fn malformed_dataset_errors() {
        let dir = std::env::temp_dir().join("attestar_cli_tests_dataset");
    ...
    fn missing_dataset_is_io_error() {
        let path = Path::new("/nonexistent/attestar/dataset.json");
...
pub fn cmd_prove(
    ...
    let bundle = generate_proof(&model, &inputs)
        .map_err(|message| CliError::Inference(AttestarError::InvalidModel(message)))?;
...
mod tests_commands {
    ...
    fn prove_to_file_reports_the_destination() {
        let dir = std::env::temp_dir().join("attestar_cli_tests_commands");
        std::fs::create_dir_all(&dir).unwrap();
...
mod tests_args {
    ...
    fn validate_defaults_match_quantization_config() {
        let cli = Cli::try_parse_from(["attestar-prover", "validate", "model.json"]).unwrap();
    ...
    fn unknown_subcommand_is_rejected() {
        assert!(Cli::try_parse_from(["attestar-prover", "frobnicate"]).is_err());
    }

    #[test]
    fn infer_requires_input() {
        assert!(Cli::try_parse_from(["attestar-prover", "infer", "model.json"]).is_err());
    }
}
